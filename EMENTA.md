...
##### Roteiro da demo
Para exibir uma lista no aplicativo usamos o controle **ListView**. O **ListView** possui uma propriedade chamada **ItemsSource**, e é através dela que inserimos a lista de itens que desejamos exibir. Inicialmente vou fazer o uso de uma lista simples do tipo *string* que já tenho definido na viewmodel. Ao atribuir a lista de tipo simples, o resultado já aparece dentro do **ListView**. 

Dificilmente temos telas tão simples para desenvolver, então, geralmente precisamos trabalhar com tipos complexos para exibir as informações em tela. Para exemplificar isso, vou atribuir na propriedade **ItemsSource** um valor de tipo complexo, que é uma lista de objetos. Vejam que agora as informações não foram exibidas de forma legível. Por que isso aconteceu? Quando passamos para o **ListView** um objeto de tipo complexo, ele não sabe qual propriedade deve ser utilizada deste objeto para exibir em tela e mostra o nome da classe que está no contexto de binding. Então, uma maneira simples de resolver isso é fazer a sobrecarga do método ToString na classe e retornar a propriedade que desejamos que apareça em tela.

Mas o que fazer quando queremos exibir mais informações, como por exemplo, mais de um texto ou uma imagem no item da lista? Para isso, o **ListView** nos possibilita customizar o template de cada item que será exibido. Para isso, precisamos especificar um **DataTemplate** na propriedade **ItemTemplate** do **ListView**. Neste **DataTemplate** precisamos inserir o layout do item que queremos que seja exibido. Este layout chamamos de _Cell_. O Xamarin.Forms já possui algumas células pré-definidas, também conhecidas como *built-in cells*. O tipo de célula mais simples é o **TextCell**, e para utilizá-la, precisamos apenas especificar o atributo **Text** e fazer o binding da propriedade que desejarmos da classe do item. No **TextCell** também podemos inserir um outro texto a ser exibido, e fazermos isso através da propriedade **Detail**. E se quisermos exibir uma imagem? Para isso, ao invés de utilizar o **TextCell**, usamos o **ImageCell** e atribuímos a imagem na propriedade **ImageSource**.

E se quisermos inserir mais textos, como fazemos? Esses tipos de células simples não ajudam muito nisso, pois não conseguimos de forma fácil customizar sua aparência ou alterar o posicionamento dos seus elementos. Para resolver isso, precisamos renderizar no **ListView** células customizadas. Para isso, basta especificarmos o atributo **ViewCell** dentro do **DataTemplate** e então inserir nossa estrutura de layout para exibir os elementos que desejarmos. Para este exemplo, vou inserir um **StackLayout** para empilhar os elementos contidos nele. Não vou explicar detalhes sobre o **StackLayout** agora pois teremos um vídeo falando somente sobre ele. Com o **StackLayout** definido, vou inserir 3 **Labels** e fazer bindings das propriedades do item. Mas não foram todos os itens que apareceram, por quê? Por que quando utilizamos uma célula customizada no **ListView**, precisamos informar a ele que a célula tem tamanho diferente do padrão definido pelo sistema operacional. Para isso, especificamos a propriedade **HasUnevenRows** e atribuímos o valor para **true**. Como estamos usando um **StackLayout**, não precisamos definir a altura da célula, pois o **StackLayout** já irá expandir automaticamente. 

As células estão todas juntas, deixando o layout um pouco confuso. Para resolver isso, vou inserir uma margem na célula para melhorar a visualização.

A lista também pode ter um cabeçalho e rodapé definido. Para isso, basta definirmos um texto nas propriedades **Header** e **Footer**. Tanto para o **Header** quando para o **Footer**, também podemos ter um layout customizado, e para isso, vamos novamente utilizar um **DataTemplate** para estes itens. Vou mostrar isso customizando o cabeçalho, mas o mesmo se aplica ao rodapé. Podemos inserir qualquer estrutra de layout dentro destes **DataTemplates**. Isso nos possibilita criar um layout bem customizado. Aqui vou manter o layout simples, pois veremos com detalhes sobre os layouts em outros vídeos. 

Veja que podemos definir o atributo **Header** e também seu template, mas afinal, para que definí-los juntos? Quando especificamos um **DataTemplate** no cabeçalho, o atributo **Header** funciona como o contexto de binding que o template usará. Podemos ver isso de forma clara quando aplicamos no **Label** o binding para o objeto atual com o valor "Binding ." na propriedade **Text**. 

As informações nas listas podem ser agrupadas, mas para isso, precisamos preparar nossa lista de objetos para representar essas informações de forma agrupada. Em nosso exemplo, vou realizar o binding para uma propriedade chamada **ListaAgrupada**. Essa propriedade contém uma lista de grupos, e em cada um destes grupos uma lista de itens. Ao fazermos este binding, as informações da lista parecem sumir. Na verdade, as inforamções continuam lá, mas agora estamos listando os grupos da lista ao invés de seus itens. Para fazer o **ListView** entender que agora estamos trabalhando com uma estrutura de lista agrupada, precisamos especificar a propriedade **IsGroupingEnabled** e configurar o valor **True**. Desta forma, passamos a ver os itens da lista e seus grupos.

Nós também podemos especificar um **DataTemplate** para os grupos da lista. Para isso, customizamos o **DataTemplate** através da propriedade **GroupHeaderTemplate** do **ListView**. Aqui neste template temos como contexto de binding o objeto do grupo, então, podemos fazer binding para as propriedades contidas neste objeto.

Quando trabalhamos com listas, é normal realizarmos algumas ações em seus itens, como favoritar ou excluir. Para isso, temos nas listas o que chamamos de ações de contexto, onde conseguimos interagir com cada item para realizar uma ação na tela. O comportamento dessa ação na tela difere entre o Android e iOS, pois cada plataforma tem as suas características. No Android clicamos e seguramos o item e as ações de contexto aparecem na barra de navegação. Já no iOS realizamos um gesto na tela arrastando o item da direita para a esquerda para aparecer as ações de contexto. Para adicionar uma ação de contexto em um item, basta especificarmos o **ContextActions** dentro do **ViewCell** do item. Este **ContextActions** espera uma coleção de ações, então, especificamos cada ação utilizando o **MenuItem**, inserindo o nome da ação através do atributo **Text**. Existe também uma propriedade chamada **IsDestructive** que irá configurar o botão de contexto com cor vermelha. Essa propriedade funciona apenas no iOS, pois o Android possui uma forma diferente de lidar com as ações de contexto. Geralmente essa opção é utilizada para ações que significam exclusões e tem como finalidade estética apenas. 

Quando clicamos em uma ação, um evento e um comando são invocados, e podemos utilizar eles para capturar a ação em nosso aplicativo. Agora vou demonstrar como realizar isso através do evento para simplificar a explicação. Quando falarmos sobre mudança de contexto de binding, ficará mais fácil entender como aplicar a mesma solução usando o comando. Para capturar o evento de clique, basta assinarmos o evento **Clicked**. Esse evento nos retorna o **MenuItem** clicado e também um objeto **EventArgs**, mas nenhum destes atributos nos diz a respeito do item clicado. Para obtermos essa informação, precisamos adicionar no **MenuItem** o contexto de binding do item clicado através da propriedade **CommandParameter**. Com isso, conseguimos descobrir na propridade **sender** do método do evento qual é o item que foi clicado. Assim, conseguiremos implementar em nosso app alguma ação referente ao item clicado.

Podemos também realizar a atualização da lista utilizando um recurso chamado "pull to refresh". Este recurso consiste em arrastarmos a lista de cima para baixo. Com isso, aparecerá um indicador visual na tela informando que a lista está sendo atualizada. Para usar este recurso, setamos na propriedade **IsPullToRefreshEnabled** o valor **true** e assim já conseguimos realizar o _swipe_ na lista. Mas apenas isso não basta para atualizá-la. Precisamos informar para o **ListView** qual é o comando que fará isso. Fazemos isso através da propriedade **RefreshCommand** e especificamos o comando que atualizará a lista na tela. Você pode ver que a lista é atualizada, mas o indicador visual da atualização não some. Para resolver isso, precisamos especificar qual é a propriedade que diz ao **ListView** se a lista está sendo atualizada. Fazemos isso através da propriedade **IsRefreshing**, onde colocamos o binding para a propriedade que controla isso. Essa propriedade deverá ser manipulada junto do método que realiza a atualização da lista. Desta forma, podemos configurar a propriedade com o valor **true** quando o método é iniciado e **false** quando o método é finalizado. Assim o **ListView** sabe quando a atualização terminou e o indicador visual da atualização some da tela. É importante lembrar que essa propriedade precisa notificar suas mudanças, e isso pode ser feito através da interface **INotifyPropertyChanged**. Também vale lembrar que para estes tipos de listas onde as informações são alteradas dinamicamente, a lista deve ser do tipo **ObservableCollection**. Caso contrário, as informações não serão atualizadas em tela.

Outra funcionalidade importante para o **ListView** é o cache. Imagine seu aplicativo exibindo uma lista com centenas de itens. Para mostrar estes itens, o **ListView** cria uma célula para cada um deles. Você já deve estar imaginando o quão seu aplicativo pode ficar pesado com isso, sobretudo para dispositivos que possuem menos capacidade. Tem também o fato de não conseguirmos exibir muitos itens na tela, pois geralmente, telas de dispositivos móveis costumam ser pequenas. Então, quando rolamos uma tela de baixo para cima, o item de cima sai da visualização da tela para o item de baixo poder aparecer. O cache no **ListView** consiste em reutilizar a instância de célula do item que está desaparecendo para o item que está aparecendo. Para isso, utilizamos a propriedade **CachingStrategy**. Seu valor padrão é **RetainElement**, que faz instanciar uma célula para cada item. Para utilizarmos o cache nas células, configuramos a propriedade com o valor **RecycleElement**. Além das células, podemos também realizar cache dos **DataTemplates** do **ListView**. Para isso, configuramos a propriedade com o valor **RecycleElementAndDataTemplate**.

Listas podem nos levar a outras telas do aplicativo. Isso acontece quando clicamos sobre um item da lista e o aplicativo é direcionado para outra tela. Aqui no **ListView** podemos fazer isso assinando o evento **ItemSelected**. Neste evento temos acesso ao **SelectedItemChangedEventArgs** e conseguimos descobrir na propriedade **SelectedItem** do mesmo qual é o item selecionado da lista. Com isso, conseguimos navegar para outra tela passando as informações do item selecionado por parâmetro.

##### Roteiro da finalização
O que você achou do **ListView**? Conta para a gente aí nos comentários e não esquece de dar aquele like no vídeo. Um abraço e até a próxima!
